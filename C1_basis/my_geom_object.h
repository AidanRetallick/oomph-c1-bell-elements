...
public:
 ...
 // double atan2cust(const double& y, const double& x) const
 // {
 //  double result = atan2(y,x);
 //  if(result<-MathematicalConstants::Pi/2.0)
 //   {result += 2.0*MathematicalConstants::Pi;}
 //  return result;
 // }
 ...
 // The arc length (parametric parameter) for the right semi circular arc
  return (Clockwise_zeta ? atan2(Y,X) : atan2(Y,X));
 }
...
/// Specialisation of CurvilineGeomObject for half a circle.
class CurvilineCircleLeft : public CurvilineGeomObject
{
public:
 /// \short Constructor: Pass dimension of geometric object (# of Eulerian
 /// coords = # of Lagrangian coords; no time history available/needed)
 CurvilineCircleLeft() : CurvilineGeomObject(), Radius(1.0), 
   Clockwise_zeta(false)
  { }

 /// \short Constructor: Pass dimension of geometric object (# of Eulerian
 /// coords = # of Lagrangian coords; no time history available/needed)
 CurvilineCircleLeft(const double& cx,
		     const double& cy,
		     const double& radius,
		     const bool& clockwise_zeta=false) : 
  CurvilineGeomObject(), Cx(cx), Cy(cy), Radius(radius), Clockwise_zeta(clockwise_zeta)
 ...
 /// of veloc/acceleration etc. in cases where the GeomData
 /// varies with time.
 CurvilineCircleLeft(TimeStepper* time_stepper_pt) : CurvilineGeomObject(time_stepper_pt)
   { }

 /// Broken copy constructor
 CurvilineCircleLeft(const CurvilineCircleLeft& dummy) 
  { 
   BrokenCopy::broken_copy("CurvilineCircleLeft");
  } 
 
 /// Broken assignment operator
 void operator=(const CurvilineCircleLeft&) 
  {
   BrokenCopy::broken_assign("CurvilineCircleLeft");
  }

 /// (Empty) destructor
 virtual ~CurvilineCircleLeft(){}
 ...
 /// Special atan2 with branch cut at zeta=0 to guarentee continuity on left
 ...
 double get_zeta(const Vector<double>& x) const 
 {
  double X=x[0]-Cx;
  double Y=x[1]-Cy;
 // The arc length (parametric parameter) for the left circular arc
  return (Clockwise_zeta ? atan2cust(Y,X) : atan2cust(Y,X));
 }
